...
- Independent ready phases become separate Jobs. Four worker threads run Jobs concurrently by default.
...
- Agent changes become an explicit commit.
- Integration uses serialized `git cherry-pick`.
...
- Discovery: bounded file listing, line-range reads, ripgrep search, symbol definition search.
...
- Search providers: Brave API, SearXNG, then Bing RSS fallback.
...
- Final synthesis also renders HTML, DOCX, PDF, `ARTIFACTS.json`, and XLSX for financial models.
